...
def tenant_appointments(
    ...
    """
    Display all appointments belonging to the authenticated tenant.
    """
    ...
    if not user_id:
        return RedirectResponse(
            url="/login?next=/tenant/appointments",
            status_code=status.HTTP_303_SEE_OTHER,
        )
    ...
    appointments = (
        ...
        .filter(Appointment.requested_by_id == current_user.id)
        .order_by(
...
def tenant_appointment_details(
    appointment_id: str,
    request: Request,
    db: Session = Depends(get_db),
):
    """
    Display one appointment belonging to the authenticated tenant.
    """
    ...
    if not user_id:
        return RedirectResponse(
            url=f"/login?next=/tenant/appointments/{appointment_id}",
            status_code=status.HTTP_303_SEE_OTHER,
        )
